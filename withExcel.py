...
def open_x():
    global xfile
    xfile = filedialog.askopenfilename(title='open a file', filetypes=(('excel files', '*.excel')))

    filename = ''
    if len(ntpath.basename(xfile)) > 25:
        filename = ntpath.basename(xfile[:25])+'...'
    else:
        filename = ntpath.basename(xfile)
    lbl_filename.config(text=filename, fg='grey')
...
#worry about verifying later
def verify_columns():
    #csv_file
    with open(xfile) as csvfile:
...
#category to add helper function
def row_condition(row):
    # may need to change 'str(row[4]) to actual row name'
    if '(M)' in str(row[4]) or '12 services' in str(row[4]) or '24 services' 
    in str(row[4]) or 'Transcend Annual Non-Medical Combo Credit' in 
    str(row[4]) or 'Transcend Annual Medical Combo Credit' in str(row[4]) or 
    'Nourish Annual Medical Credit' in str(row[4]):
        if str(row[1]) in member_IDs:
            return 'Membership - Renewal'
        else:
            member_IDs.append(str(row[1]))
            return 'Membership - New'
    elif '(A)' in str(row[4]) or 'Service Credit' in str(row[4]) or
    'Acupuncture' in str(row[4]) or '2 services' in str(row[4]) or '1 service' 
    in str(row[4]) or 'Complimentary Consultation' in str(row[4]) or 
    'Appointment' in str(row[4]) or 'Sacred Sunday Members' in str(row[4]): 
        return 'Appointment'
        
    elif '(E)' in str(row[4]) or 'Moon Circle' in str(row[4]) or 'Sunset 
    Horseback Riding' in str(row[4]) or 'Sunset Turtle Releasing Ceremony' in 
    str(row[4]) or 'Getaway' in str(row[4]) or 'Drop In' in str(row[4]) or 
    'Reiki'  in str(row[4]) or 'Sacred Sunday Drop In' in str(row[4]) or 
    'Sacred Sunday Non-Member' in str(row[4]): 
        return 'Event'
    elif '(R)' in str(row[4]) or '(S)' in str(row[4]):
        return 'retail'
    elif '(C)' in str(row[4]):
        return 'Corporate Event'
    elif '(D)' in str(row[4]) or 'Internal Meeting Non Member' in str(row[4]) 
    or '$150 Membership Pricing' in str(row[4]) or '$125 Membership Pricing' 
    in str(row[4]) or '$100 Membership Pricing' in str(row[4]):
        return 'DELETE'
...
def categorize_revenue():
    # still need to verify valid EXCEL

    df = pd.read_excel("VERIFIED EXCEL FILE")
    cat_column = []
...
btn_choosefile = Button(master=frame2,text='Choose File',command=file_type() #open_csv)
